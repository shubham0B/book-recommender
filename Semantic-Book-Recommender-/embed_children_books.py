...
def is_clean_children_title(title: str) -> bool:
...
def fetch_children_from_local_csv(genre_type: str, existing_ids: Set[str], existing_keys: Set[str], target: int) -> List[Dict[str, Any]]:
    collected = []
    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            if not is_clean_children_title(title):
                continue
            ...
            prefix = "cf" if genre_type == "Children's Fiction" else "cnf"
            doc_id = f"{prefix}_{isbn_13 or idx}"
            ...
            semantic_doc = build_semantic_text(title, authors_str, genre_type, raw_cat or genre_type, desc_clean)
            ...
            book_item = {
                ...
                "categories": raw_cat if raw_cat != "nan" else genre_type,
                "genre": genre_type,
        ...
        print(f"Local extraction notice: {e}")

    print(f"[{genre_type}] Extracted {len(collected)} books from local CSV.")
    return collected

def fetch_children_from_openlibrary(genre_type: str, subjects: List[str], existing_ids: Set[str], existing_keys: Set[str], target: int) -> List[Dict[str, Any]]:
    collected = []
        ...
        for page in range(1, 15):
            ...
            try:
                ...
                for d in docs:
                    ...
                    prefix = "ol_cf" if genre_type == "Children's Fiction" else "ol_cnf"
                    doc_id = f"{prefix}_{d.get('key', '').replace('/', '_') or cover_i}"
                    dedup_key = make_dedup_key(title, authors_str)
                    ...
                    desc = f"A delightful and engaging book for young readers about {subj_str.lower()} by {authors_str}."
...
def fetch_children_from_google_books(genre_type: str, queries: List[str], existing_ids: Set[str], existing_keys: Set[str], target: int) -> List[Dict[str, Any]]:
    collected = []
        ...
        for start_idx in range(0, 80, 40):
            ...
            try:
                ...
                for item in items:
                    ...
                    gb_id = item.get("id", "")
                    prefix = "gb_cf" if genre_type == "Children's Fiction" else "gb_cnf"
                    doc_id = f"{prefix}_{gb_id}" if gb_id else f"isbn_{isbn_13 or isbn_10 or make_dedup_key(title, authors_str)}"
                    dedup_key = make_dedup_key(title, authors_str)
                    ...
                    clean_desc = normalize_string(raw_desc) or f"A wonderful {genre_type.lower()} book for children by {authors_str}."
...
def embed_genre_books(genre_name: str, target_count: int, subjects: List[str], google_queries: List[str], batch_size: int = 64):
    print("==================================================")
    ...
    # 1. Local CSV
    local_b = fetch_children_from_local_csv(genre_name, existing_ids, existing_keys, target=needed)
    records.extend(local_b)

    # 2. OpenLibrary
    rem = needed - len(records)
    if rem > 0:
        ol_b = fetch_children_from_openlibrary(genre_name, subjects, existing_ids, existing_keys, target=rem)
        records.extend(ol_b)

    # 3. Google Books
    rem = needed - len(records)
    if rem > 0:
        gb_b = fetch_children_from_google_books(genre_name, google_queries, existing_ids, existing_keys, target=rem)
        records.extend(gb_b)
...
def run():
    # 1. Children's Fiction (2000 books)
    cf_subjects = [
        "juvenile_fiction", "picture_books", "children's_stories", "fairy_tales",
        "fables", "children_adventure", "children_fantasy", "bedtime_stories",
        "animals_fiction", "caldecott_medal", "newbery_medal", "early_readers",
        "chapter_books", "folk_tales"
    ]
    cf_google = [
        'subject:"juvenile fiction"', 'subject:"children\'s stories"', 'subject:"picture books"',
        'inauthor:"Roald Dahl"', 'inauthor:"Dr. Seuss"', 'inauthor:"Beatrix Potter"',
        'inauthor:"E.B. White"', 'inauthor:"Maurice Sendak"', 'inauthor:"Eric Carle"',
        'inauthor:"Beverly Cleary"', 'inauthor:"Astrid Lindgren"', 'inauthor:"C.S. Lewis" Narnia',
        'inauthor:"Lewis Carroll"', 'inauthor:"A.A. Milne"', 'inauthor:"Shel Silverstein"',
        'inauthor:"Rick Riordan"', 'inauthor:"Jeff Kinney"', 'inauthor:"Dav Pilkey"',
        'inauthor:"Julia Donaldson"', 'inauthor:"Kate DiCamillo"', 'inauthor:"Lemony Snicket"'
    ]
    embed_genre_books("Children's Fiction", 2000, cf_subjects, cf_google)

    # 2. Children's Nonfiction (2000 books)
    cnf_subjects = [
        "juvenile_nonfiction", "science_juvenile_literature", "animals_juvenile_literature",
        "space_juvenile_literature", "biography_juvenile_literature", "history_juvenile_literature",
        "geography_juvenile_literature", "nature_juvenile_literature", "dinosaurs_juvenile_literature",
        "inventions_juvenile_literature", "technology_children", "math_children", "art_children",
        "children_encyclopedias", "oceans_juvenile_literature", "weather_juvenile_literature"
    ]
    cnf_google = [
        'subject:"juvenile nonfiction"', 'subject:"science for kids"', 'subject:"history for kids"',
        'subject:"nature for kids"', 'subject:"space for kids"', 'subject:"animals for kids"',
        '"National Geographic Kids"', '"DK Eyewitness"', '"Magic School Bus"',
        '"Who Was" biography', '"Little People, BIG DREAMS"', 'subject:"dinosaurs for kids"',
        'subject:"biography for children"', 'subject:"geography for children"'
    ]
    embed_genre_books("Children's Nonfiction", 2000, cnf_subjects, cnf_google)
