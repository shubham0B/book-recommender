...
def resolve_authentic_cover(title: str, author: str, isbn_13: str = "", isbn_10: str = "", google_thumb: str = "") -> str:
    """Resolve the authentic real book cover photo from OpenLibrary or Google Books."""
    is_archive_scan = "printsec=frontcover" in google_thumb or "source=gbs_api" in google_thumb
    ...
    # Try OpenLibrary search first for high quality retail photo
    try:
        resp = requests.get(
        ...
        if resp.status_code == 200:
            ...
            for d in docs:
                ...
                isbns = d.get("isbn", [])
                if isbns:
                    real_cover = f"https://covers.openlibrary.org/b/isbn/{isbns[0]}-L.jpg"
                    _COVER_CACHE[cache_key] = real_cover
                    return real_cover
    except Exception:
        pass

    # If not found on OpenLibrary, only use Google Thumb if it is not a plain archive scan
    if google_thumb and not is_archive_scan and "placeholder" not in google_thumb:
        return google_thumb
