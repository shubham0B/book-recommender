...
// OpenLibrary Direct API Search (Instant fallback when backend or Google Books API is unavailable)
async function searchOpenLibraryBooks(query: string, targetGenres: string[] = []): Promise<Book[]> {
  try {
    ...
    const res = await fetch(
      `https://openlibrary.org/search.json?q=${encodeURIComponent(clean)}&limit=36&fields=title,author_name,cover_i,isbn,subject,first_sentence,subtitle,key`,
    ...
    const mapped: Book[] = docs
      .filter((d: any) => d.title && !isBestsellerTitle(d.title))
      .map((d: any) => {
        ...
        let detectedGenre = 'Literature';
        if (d.subject && Array.isArray(d.subject)) {
          const subjectsStr = d.subject.join(' ').toLowerCase();
          for (const g of ALL_GENRES) {
            if (subjectsStr.includes(g.toLowerCase())) {
              detectedGenre = g;
              break;
            }
          }
        ...
        const firstSent = Array.isArray(d.first_sentence) ? d.first_sentence[0] : (d.first_sentence || '');
        const desc = firstSent || (d.subtitle ? `${d.title}: ${d.subtitle}. By ${author}.` : `A celebrated work by ${author}.`);
        ...
        return {
          ...
          genre: detectedGenre,
          rating: 4.5,
          isbn: isbn || String(coverId || '0'),
          description: desc,
          thumbnail: thumb,
          source: 'OpenLibrary',
          links: {
            info: d.key ? `https://openlibrary.org${d.key}` : `https://openlibrary.org/search?q=${encodeURIComponent(d.title)}`,
            preview: d.key ? `https://openlibrary.org${d.key}` : undefined
          }
        };
      });
    ...
    console.warn('OpenLibrary search fallback error:', err);
    return [];
  }
}

async function fetchOpenLibrarySuggestions(query: string): Promise<SuggestionItem[]> {
  try {
    const clean = query.trim();
    if (!clean) return [];
    const controller = new AbortController();
    ...
    const res = await fetch(
      `https://openlibrary.org/search.json?q=${encodeURIComponent(clean)}&limit=8&fields=title,author_name,cover_i,isbn`,
      { signal: controller.signal }
    );
    clearTimeout(timeoutId);
    if (!res.ok) return [];
    ...
    return docs
      ...
      .map((d: any) => ({
        ...
        genre: 'Literature',
        thumbnail: d.cover_i ? `https://covers.openlibrary.org/b/id/${d.cover_i}-S.jpg` : (d.isbn?.[0] ? `https://covers.openlibrary.org/b/isbn/${d.isbn[0]}-S.jpg` : ''),
        isbn: d.isbn?.[0] || ''
      }));
  } catch {
    return [];
...
export function BookMindApp() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      // Fetch suggestions from Backend, fallback to OpenLibrary
      try {
        const controller = new AbortController()
        const timeoutId = setTimeout(() => controller.abort(), 2500)
        const res = await fetch(`https://web-production-9205dd.up.railway.app/api/books/suggest?q=${encodeURIComponent(trimmed)}`, {
        ...
        if (res.ok) {
          ...
          if (data.suggestions && data.suggestions.length > 0) {
            ...
            const olSuggests = await fetchOpenLibrarySuggestions(trimmed)
            const merged = [...olSuggests, ...clientMatches]
      ...
      // 2. Perform live search in background with active genre filters
      ...
      try {
        ...
        const res = await fetch(`https://web-production-9205dd.up.railway.app/api/books/search?q=${encodeURIComponent(trimmed)}&category=${encodeURIComponent(category)}&genres=${genresParam}`, {
          headers: { 'Bypass-Tunnel-Reminder': 'true' },
        ...
        if (res.ok) {
          ...
          if (booksList && booksList.length > 0) {
            setApiBooks(booksList.map((r: any) => ({
              title: r.title, author: r.authors || r.author, genre: r.genre || 'Literature',
            ...
            setIsExternalSearch(Boolean(data.external_results?.length > 0))
            setSearchStats({ total: data.total_results || booksList.length, local: data.sources?.local || 0, external: data.sources?.google_books || data.sources?.open_library || 0 })
            return
          }
        }
        // Fallback to OpenLibrary if backend returns empty or error
        const olBooks = await searchOpenLibraryBooks(trimmed, activeGenresList)
        if (olBooks.length > 0) {
          setApiBooks(olBooks)
          setIsExternalSearch(true)
          setSearchStats({ total: olBooks.length, local: 0, external: olBooks.length })
        } else {
          const localMatches = books.filter(b => b.title.toLowerCase().includes(trimmed.toLowerCase()) || b.author.toLowerCase().includes(trimmed.toLowerCase()))
          setApiBooks(localMatches)
        }
      } catch {
        const olBooks = await searchOpenLibraryBooks(trimmed, activeGenresList)
        if (olBooks.length > 0) {
  ...
  const handleSearch = async (e?: React.FormEvent, overrideCat?: string, customQuery?: string, overrideGenres?: string[]) => {
    ...
    if (currentQuery && currentQuery.toLowerCase() !== 'popular') {
      try {
        ...
        const res = await fetch(`https://web-production-9205dd.up.railway.app/api/books/search?q=${encodeURIComponent(currentQuery)}&category=${encodeURIComponent(currentCategory)}&genres=${genresParam}&limit=120`, {
          headers: { 'Bypass-Tunnel-Reminder': 'true' },
          signal: controller.signal
        })
        clearTimeout(timeoutId)
        ...
        // Direct OpenLibrary Fallback
        const olBooks = await searchOpenLibraryBooks(currentQuery, activeGenresList)
        if (olBooks.length > 0) {
          setApiBooks(olBooks)
          setIsExternalSearch(true)
          setSearchStats({ total: olBooks.length, local: 0, external: olBooks.length })
        } else {
          const localMatches = books.filter(b => b.title.toLowerCase().includes(currentQuery.toLowerCase()) || b.author.toLowerCase().includes(currentQuery.toLowerCase()))
          setApiBooks(localMatches)
          setIsExternalSearch(false)
          setSearchStats({ total: localMatches.length, local: localMatches.length, external: 0 })
        }
      } catch (err) {
        console.warn('Search backend error, falling back to OpenLibrary:', err)
        const olBooks = await searchOpenLibraryBooks(currentQuery, activeGenresList)
        if (olBooks.length > 0) {
          setApiBooks(olBooks)
          setIsExternalSearch(true)
      ...
      try {
        const controller = new AbortController()
        const timeoutId = setTimeout(() => controller.abort(), 3500)
        const res = await fetch('https://web-production-9205dd.up.railway.app/recommend', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (res.ok) {
          ...
          if (booksList && booksList.length > 0) {
            const valid = booksList.map((r: any) => ({
              title: r.title, author: r.authors || r.author, genre: r.genre || r.simple_categories || currentCategory,
              rating: r.rating !== undefined && r.rating !== null ? Number(r.rating) : 4.0, isbn: r.isbn || '0', description: r.description, thumbnail: r.thumbnail,
              source: r.source || 'BookMind Library', links: r.links
            }))
            setApiBooks(valid)
        ...
        {!isLoading && visibleBooks.length === 0 && (
          <div className="flex flex-col items-center gap-4 rounded-3xl border border-border bg-card px-6 py-16 text-center">
            ...
            <h3 className="font-serif text-2xl font-semibold">
              {query ? `No books found for “${query}”` : 'No books found for the selected genre'}
            </h3>
            <p className="text-muted-foreground max-w-md text-sm">
              {query 
                ? `We couldn't find any books matching “${query}”. Try searching for another book title, author, or keyword.`
                : "We couldn't find any books matching your strict genre filter. Try exploring other genres or adjusting your search."}
            </p>
            <div className="flex gap-3">
              <Button onClick={() => { setQuery(''); setCategory('All'); setSelectedGenres([]); handleSearch(undefined, 'All', 'popular', []); }}>
                Browse All Books
              </Button>
            </div>
          </div>
